...
for i = m1 + (W + 1)/2  : m2 - (W + 1)/2 
    ...
    %循环寻找局域最大值
    if y_hat(i)==max(y_hat(i-(W-1)/2:i+(W-1)/2))
        s_p(count_p,1)=i;
        s_p(count_p,2)=h_in(i);
        count_p=count_p+1;
    end
end
s(count,1) = m2;
...
D_up=mean(s_p(:,2));%upper threshold value
D_thr=lower_value(img,D_up);
...
count_v=1;
for gamma=0.1:0.1:5
    F_temp(count_v,1)=gamma;
    F_temp(count_v,2)=entropy(Gamma_correction(F,gamma));
    count_v=count_v+1;
end
[entro,count_v]=max(F_temp(:,2));
F=Gamma_correction(F,F_temp(count_v,1));

toc;
figure(2);
imshow(F);
imwrite(F,strcat('A',path));
